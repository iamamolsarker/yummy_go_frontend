...
/* ---------------- Types ---------------- */
// ✅ FIX: Use statuses from your backend controller
type RestaurantStatus = "pending" | "approved" | "rejected" | "suspended" | "active" | "open" | "closed";
...
interface Restaurant {
  _id: string;
  name: string;
  owner?: string; // This seems to be a UI-only field, not in your backend model
  email?: string;
  phone?: string;
  cuisine?: string; // Your model has `cuisine: []` but frontend uses `string`. Allowing string for now.
  address?: string; // Your model has `location: { address: ... }`
  photoURL?: string;
  location?: Location;
  status: RestaurantStatus;
  rating?: number;
  reviews_count?: number; // Your model has `total_reviews`
  is_verified?: boolean;
  created_at?: string;
  // UI-only
  isDropdownOpen?: boolean;
...
const getStatusDisplay = (status?: RestaurantStatus) => {
  switch (status) {
    ...
    case "active":
    case "approved":
      return { text: status.charAt(0).toUpperCase() + status.slice(1), className: "bg-green-100 text-green-800" };
    case "closed":
      return { text: "Closed", className: "bg-gray-100 text-gray-800" };
    case "pending":
      return { text: "Pending", className: "bg-yellow-100 text-yellow-800" };
    case "suspended":
    case "rejected":
      return { text: status.charAt(0).toUpperCase() + status.slice(1), className: "bg-red-100 text-red-800" };
    default:
      return { text: "Unknown", className: "bg-gray-200 text-gray-800" };
  }
};
...
// ✅ FIX: Define valid statuses for tabs based on your backend routes
const TABS: ("All" | RestaurantStatus)[] = ["All", "pending", "approved", "active", "suspended", "rejected"];
...
export default function RestaurantManagement() {
  ...
  /* ✅ FIX: Fetch restaurants based on statusFilter, matching your backend API */
  useEffect(() => {
    const fetchRestaurants = async () => {
      try {
        ...
        // Determine the correct API endpoint based on the filter
        let url = "/restaurants";
        if (statusFilter !== "All") {
          // Use the specific status endpoint from your restaurantRoutes.js
          url = `/restaurants/status/${statusFilter}`;
        }
        
        // Note: Your backend GET /restaurants (for 'All') is filtered by 'is_active: true'.
        // You may want to remove that filter in your Restaurant.findAll model for the admin panel.

        const res = await axiosSecure.get(url);
        
        // server might return array or {data: []} or {data: { restaurants: [] }}
        const data = Array.isArray(res.data) 
          ? res.data 
          : (Array.isArray(res.data.data) ? res.data.data : (res.data.data?.restaurants || []));

        // ensure status default
        const normalized: Restaurant[] = data.map((r: any) => ({
          ...r,
          status: r.status || "pending",
          reviews_count: r.total_reviews || 0, // Map backend 'total_reviews'
          address: r.location?.address || r.address, // Normalize address
        }));
    ...
    fetchRestaurants();
  }, [axiosSecure, statusFilter]); // Re-fetch when statusFilter changes
  ...
  /* ✅ FIX: Change status - Use PATCH /restaurants/:id/status */
  const handleStatusChange = async (id: string, status: RestaurantStatus) => {
    const prev = restaurants.find((r) => r._id === id);
    updateRestaurantLocal(id, { status, isDropdownOpen: false });
    try {
      // This matches: router.patch('/:id/status', restaurantController.updateRestaurantStatus);
      await axiosSecure.patch(`/restaurants/${id}/status`, { status });
    } catch (err: any) {
      // rollback
      if (prev) updateRestaurantLocal(id, { status: prev.status });
      setError(err.response?.data?.message || "Failed to update status.");
  ...
  /* ✅ FIX: Delete (soft) - DELETE /restaurants/:id (This was already correct) */
  const handleDelete = async (id: string) => {
    const prev = restaurants;
    setRestaurants((p) => p.filter((r) => r._id !== id));
    try {
      // This matches: router.delete('/:id', restaurantController.deleteRestaurant);
      await axiosSecure.delete(`/restaurants/${id}`);
    } catch (err: any) {
      setRestaurants(prev);
      setError(err.response?.data?.message || "Failed to delete restaurant.");
  ...
  /* ✅ FIX: Update rating - PATCH /restaurants/:id/rating */
  // Backend expects 'total_reviews', frontend type has 'reviews_count'
  const handleRatingUpdate = async (id: string, rating: number, reviews_count: number) => {
    const prev = restaurants.find((r) => r._id === id);
    updateRestaurantLocal(id, { rating, reviews_count });
    try {
      // This matches: router.patch('/:id/rating', restaurantController.updateRestaurantRating);
      // We send 'total_reviews' as expected by the backend controller
      await axiosSecure.patch(`/restaurants/${id}/rating`, { 
        rating, 
        total_reviews: reviews_count 
      });
    } catch (err: any) {
      if (prev) updateRestaurantLocal(id, { rating: prev.rating, reviews_count: prev.reviews_count });
      setError(err.response?.data?.message || "Failed to update rating.");
    }
  };

  /* ✅ FIX: Generic Update - Use PUT /restaurants/:id */
  const handleUpdate = async (id: string, payload: Partial<Restaurant>) => {
    const prevItem = restaurants.find((r) => r._id === id);
    updateRestaurantLocal(id, payload);
    ...
    try {
      ...
      // This matches: router.put('/:id', restaurantController.updateRestaurant);
      const res = await axiosSecure.put(`/restaurants/${id}`, cleaned);
    ...
    <div className="bg-gray-50 min-h-screen p-6">
                  ...
                  <tr key={r._id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          <div className="text-sm font-semibold text-gray-900">{r.name}</div>
                          {/* Owner is not in restaurant model, so we show email/phone */}
                          <div className="text-sm text-gray-500">{r.email || "N/A"}</div>
                          <div className="text-xs text-gray-400">{r.address || "Address not set"}</div>
                        </div>
                      </div>
                    </td>
                      ...
                      <div className="flex items-center justify-center gap-2 flex-wrap">
                        ...
                        <div className="relative">
                          ...
                          {r.isDropdownOpen && (
                            <div className="absolute right-0 mt-1 flex flex-col bg-white border border-gray-200 rounded-md shadow-lg w-36 z-10 animate-fade-in">
                              {/* ✅ FIX: Use statuses from your backend controller */}
                              {(["pending", "approved", "rejected", "suspended", "active"] as const).map((s) => (
                                <button
                                  key={s}
                                  onClick={() => handleStatusChange(r._id, s)}
                                  className="px-3 py-2 text-sm text-left hover:bg-gray-100 capitalize transition"
      ...
      {modal.restaurant && (
        <RestaurantActionModal
          ...
          // ✅ FIX: Pass the updated rating function
          onUpdateRating={(id, rating, reviews_count) => {
            handleRatingUpdate(id, rating, reviews_count);
            closeModal();
          }}
        />
      )}
...
function RestaurantActionModal({
  ...
  onDelete: (id: string) => void;
  onUpdate: (id: string, payload: Partial<Restaurant>) => void;
  // ✅ FIX: Update signature to include reviews_count
  onUpdateRating: (id: string, rating: number, reviews_count: number) => void;
}) {
  const restaurant = modal.restaurant;
  if (!restaurant) return null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // normalize numeric fields
    if (payload.rating !== undefined) payload.rating = parseFloat(payload.rating);
    // ✅ FIX: Normalize new reviews_count field
    if (payload.reviews_count !== undefined) payload.reviews_count = parseInt(payload.reviews_count, 10);
    if (payload.lat !== undefined) payload.lat = parseFloat(payload.lat);
    if (payload.lng !== undefined) payload.lng = parseFloat(payload.lng);

    if (modal.type === "rating") {
      // ✅ FIX: Pass both values, matching the backend controller's needs
      onUpdateRating(restaurant._id, payload.rating, payload.reviews_count);
    } else if (modal.type === "location") {
      onUpdate(restaurant._id, { location: { lat: payload.lat, lng: payload.lng } });
    } else if (modal.type === "edit") {
      const updated: Partial<Restaurant> = {
        ...
        phone: payload.phone,
        cuisine: payload.cuisine,
        address: payload.address, // This will be sent as a top-level field
        email: payload.email,
        // Note: Your backend PUT /:id updates 'location' object separately
        // This simple form will send 'address' as a string, not location.address
      };
      onUpdate(restaurant._id, updated);
    } else if (modal.type === "delete") {
      onDelete(restaurant._id);
    ...
    <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md animate-fade-in">
        ...
        {modal.type === "delete" ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* ✅ FIX: Update Rating modal to include total_reviews (as reviews_count) */}
            {modal.type === "rating" && (
              <>
                <div>
                  <label className="text-sm font-medium text-gray-600">Rating (0-5)</label>
                  <input
                    name="rating"
                    type="number"
                    step="0.1"
                 ...
                 <div>
                  ...
                  <input
                    ...
                    step="1"
                    min="0"
                    defaultValue={restaurant.reviews_count ?? 0}
                    className="w-full border rounded-lg px-3 py-2 mt-1"
            ...
            {modal.type === "edit" && (
              <>
                {/* ✅ FIX: Removed 'owner' field as it's not in the restaurant model */}
                <input name="name" defaultValue={restaurant.name} className="w-full border rounded-lg px-3 py-2" placeholder="Name" required />
                <input name="email" defaultValue={restaurant.email} className="w-full border rounded-lg px-3 py-2" placeholder="Email" />
                <input name="phone" defaultValue={restaurant.phone} className="w-full border rounded-lg px-3 py-2" placeholder="Phone" />
                <input name="cuisine" defaultValue={restaurant.cuisine} className="w-full border rounded-lg px-3 py-2" placeholder="Cuisine" />
                <input name="address" defaultValue={restaurant.address} className="w-full border rounded-lg px-3 py-2" placeholder="Address (street)" />
              </>
            )}
